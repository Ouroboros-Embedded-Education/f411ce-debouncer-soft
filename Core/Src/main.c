...
volatile uint32_t debCount = 0;
volatile uint32_t flagInterruption = 0;

volatile uint32_t flagDebEnabled = 1;
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1)
	{
		...
		if (flagInterruption == 1){
			...
			screen_upd_raw(intCount);
			screen_upd_debounced(debCount);
		}
    /* USER CODE END WHILE */
